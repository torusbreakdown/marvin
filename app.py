    ...
    async def _code_review(phase_label: str, focus: str = "all") -> None:
        ...
        def _build_review_prompt(area: str, files: list[str]) -> str:
            ...
            if area == "Backend":
                ctx += (
                    ...
                    "Focus on: backend integration, database correctness, API contract "
                ...
                ctx += (
                    "\nFocus on: test coverage gaps, unnecessary mocking (tests should use real "
                    "implementations — flag any mock of a module/service that exists on the system), "
                    "fixture setup, assertion specificity, tests that assert buggy behavior.\n\n"
                )
            ctx += (
        ...
        if all_project_files:
            ...
            if is_test_only_phase:
                spec_review_ctx += (
                    "IMPORTANT: This is a TEST-WRITING phase (TDD). Implementation code does NOT exist yet — "
                    "that is EXPECTED and CORRECT. Do NOT report missing implementation as an issue. "
                    "Your job is ONLY to verify that the TESTS correctly cover the spec requirements. "
                    "Tests SHOULD fail right now (no implementation) — that is the point of TDD.\n\n"
                    "STEPS:\n"
                    ...
                    "   - Tests cover every spec requirement and acceptance criterion\n"
                    ...
                    "   - Tests do NOT assert placeholder/dummy behavior\n\n"
                )
            else:
                spec_review_ctx += (
    ...
    if params.tdd:
        if _phase_done("2b"):
            ...
            integ_prompt = (
                ...
                "human interaction. Use real databases, real HTTP clients, real file I/O, "
                "real subprocess calls. The only things you should mock are: user input "
                "(keyboard/mouse). If a library "
                "or service is installed on this system, USE IT FOR REAL.\n"
                "- NEVER mock anything in .marvin/upstream/. If an upstream CLI is available, "
                "test it with a real subprocess.\n"
    ...
    if _phase_done("3"):
        ...
        if os.path.isfile(design_path):
            ...
            impl_prompt += (
                ...
                "If you embed JavaScript/CSS inside a TypeScript/Python string literal (e.g. serving "
                "inline JS from a server, HTML templates with <script> blocks), ALL backslash "
                "sequences must be DOUBLE-ESCAPED. For example:\n"
                "  - A regex matching newlines must be \\\\n not \\n (\\n becomes a real newline)\n"
                "  - A regex matching tabs must be \\\\t not \\t\n"
                "  - A regex matching word boundaries must be \\\\b not \\b\n"
                "  - Backslashes in regex must be \\\\\\\\ not \\\\\n"
                "This applies to ALL code served as strings: inline <script>, template literals "
                "sent via res.send(), string constants containing JS/CSS/HTML. If the host "
                "language interprets the escape, the served code will be broken.\n\n"
