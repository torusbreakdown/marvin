...
async def launch_agent(params: LaunchAgentParams) -> str:
    ...
    async def _code_review(phase_label: str, focus: str = "all") -> None:
        ...
        def _build_review_prompt(area: str, files: list[str]) -> str:
            ...
            if area == "Backend":
                ctx += (
                    ...
                    "重点：后端集成、数据库正确性、API合约与spec匹配、"
                    "错误处理、SQL注入、子进程安全。\n\n"
                )
            elif area == "Frontend":
                ctx += (
                    "\n重点：innerHTML/renderMarkdown的XSS、SSE解析正确性、"
                    "fetch错误处理、无障碍性、键盘导航、主题切换、"
                    "响应式断点与spec匹配。\n\n"
                )
            else:
                ctx += (
                    "\n重点：测试覆盖缺口、不必要的mock（测试应使用真实实现——"
                    "标记任何对系统中存在的模块/服务的mock）、"
                    "fixture设置、断言精确度、断言错误行为的测试。\n\n"
                )
            ctx += (
                "审查标准——只标记真正的问题：\n"
                "1. **安全** — XSS、注入、不安全HTML、缺少转义\n"
                "2. **逻辑缺陷** — 条件错误、边界错误、竞态条件\n"
                "3. **集成缺陷** — 错误的端点、错误的数据结构、SSE问题\n"
                "4. **缺少错误处理** — 未处理的异常、缺少null检查\n"
                "5. **规格违反** — spec.md中有但缺失或错误的功能\n\n"
                "不要标记：代码风格、命名、缺少注释、小重构。\n\n"
                "每个问题格式：FILE:LINE — SEVERITY (critical/major/minor) — 描述\n\n"
                "以报告形式输出发现。不要编辑任何文件。不要运行命令。\n"
                "你是只读审查者——报告问题，父编排器会派遣独立代理修复。\n\n"
                "如果没有critical/major问题，回复'REVIEW_CLEAN'。"
            )
            return ctx
        ...
        if all_project_files:
            ...
            if is_test_only_phase:
                spec_review_ctx += (
                    "重要：这是测试编写阶段（TDD）。实现代码还不存在——"
                    "这是预期且正确的。不要报告缺少实现为问题。"
                    "你的工作仅是验证测试正确覆盖规格要求。"
                    "测试现在应该失败（没有实现）——这就是TDD的要点。\n\n"
                    "步骤：\n"
                    "1. 读取.marvin/spec.md——记录每个用户故事、验收标准、端点、行为。\n"
                    "2. 读取.marvin/design.md——记录测试计划部分。\n"
                    "3. 读取所有测试文件并验证：\n"
                    "   - 测试覆盖每个规格要求和验收标准\n"
                    "   - 测试验证与规格匹配的可观测行为\n"
                    "   - 测试不mock应该真实测试的东西\n"
                    "   - 测试断言匹配规格定义的值（状态码、字段名等）\n"
                    "   - 测试不断言占位符/虚拟行为\n\n"
                )
            else:
                spec_review_ctx += (
                    "你唯一的工作是验证实现与规格和架构文档完全匹配。"
                    "你不审查代码质量——只检查每个需求是否正确实现。\n\n"
                    "步骤：\n"
                    "1. 分块读取.marvin/spec.md（每次200行）——记录每个用户故事、验收标准、"
                    "端点、行为、约束和功能需求。用write_note保存发现。\n"
                    "2. 分块读取.marvin/ux.md（每次200行）——记录每个界面、组件、交互、"
                    "状态、样式规则和无障碍需求。用write_note保存发现。\n"
                    "3. 分块读取.marvin/design.md（每次200行）——记录每个架构决策、文件"
                    "结构、API形状、数据库模式和模块职责。用write_note保存发现。\n"
                    "4. 读取所有实现文件并验证：\n"
                    "   - 每个用户故事的验收标准已实现\n"
                    "   - 每个API端点匹配指定的请求/响应格式\n"
                    "   - 每个数据库表/列匹配模式\n"
            ...
            spec_review_ctx += (
                ...
                "输出格式——每个发现：\n"
                "SPEC_GAP: <规格章节> — <缺失或错误的内容> — SEVERITY: critical/major/minor\n\n"
                "以报告形式输出发现。不要编辑任何文件。不要运行命令。\n"
                "你是只读审查者——报告问题，父编排器会派遣独立代理修复。\n\n"
                "如果一切与规格匹配，回复'SPEC_CONFORMANT'。"
            )
        ...
        for review_round in range(1, _MAX_REVIEW_ROUNDS + 1):
            ...
            if is_test_phase:
                fix_instructions = (
                    ...
                    "3. 不要编写任何实现代码——不要创建server.py、app.ts或源模块\n"
                    "4. 不要试图让测试通过——测试必须失败因为实现还不存在。"
                    "这是TDD：测试在代码编写之前定义接口。\n"
                    "5. 不要运行pytest——测试会失败，这是正确的。\n"
                    "6. 如果审查者说'模块X未找到'或'import错误'，这是TDD中预期的行为。"
                    "不要创建模块。不要修复import。保持原样。\n"
                    f"7. 用消息'Test review fix ({phase_label.lower()}, round {review_round}): <summary>'提交修复\n\n"
                    "读取.marvin/spec.md、.marvin/ux.md和.marvin/design.md了解预期行为。"
                )
            else:
                fix_instructions = (
                    "指令：\nOutput in English.\n"                    "1. 读取每个发现中提到的文件\n"
                    ...
                    "5. 提交后停止。不要自我审查。不要评分。不要总结报告。\n\n"
                    "读取.marvin/spec.md、.marvin/ux.md和.marvin/design.md了解预期行为。"
                )

            fix_prompt = (
                _project_context() + "\n\n"
                f"代码审查发现（第{review_round}/{_MAX_REVIEW_ROUNDS}轮）：\n"
                "以下问题由并行代码审查者发现。"
                "修复所有critical和major问题。Minor问题可选。\n\n"
                f"{combined_findings}\n\n"
                f"{fix_instructions}"
            )
    ...
    if params.design_first:
        ...
        if _phase_done("1b") or (os.path.isfile(design_path) and os.path.getsize(design_path) > 1000):
            ...
            arch_prompt = (
                ...
                "1) 架构——文件结构、模块、数据流、API端点"
                "（请求/响应结构+状态码）、数据库模式、错误处理、技术选型及理由\n"
                "2) 实现计划——按顺序列出文件及职责+规格需求追踪\n"
                "3) 测试计划（最重要）——对每个模块/端点/组件/流程：\n"
                "   - 测试文件路径+每个测试函数名+验证内容\n"
                "   - 覆盖：正常、错误、边界、边缘、无效、空值、并发\n"
                "   - 每个公开函数、API路由、错误处理器都必须有测试\n"
                "   - 目标100%%功能覆盖率\n\n"
                "禁止Mock：使用真实数据库（SQLite临时）、真实HTTP客户端、真实文件I/O、"
                "真实子进程。只Mock用户键盘输入。不要fake fixtures，测试名中不要'mock'，"
                "不要命令参数间接调用。测试可观察行为"
                "（HTTP响应、数据库状态、渲染输出）而非内部实现。\n\n"
            )
            upstream_summary = _upstream_summary()
            if upstream_summary:
                arch_prompt += (
                    "UPSTREAM (.marvin/upstream/) = 现有系统的权威文档。"
                    "硬性需求，非建议。必须精确匹配。写入前先用read_file全部阅读。\n\n"
                )
            if instructions_ctx:
                arch_prompt += f"PROJECT INSTRUCTIONS:\n{instructions_ctx}\n\n"
            arch_prompt += (
                "保存为 .marvin/design.md。分段写入：第一段用create_file，后续用append_file"
                "（每次2000-4000词）。此文档+spec+UX=所有实现代理的唯一参考。"
                "测试计划必须详尽到代理仅凭此即可编写完整测试套件。"
            )
    ...
    if params.tdd:
        if _phase_done("2b"):
            ...
            integ_prompt = (
                ...
                "这些测试补充已有（或将要有）的功能测试。\n\n"
                "关键规则——仔细阅读：\n"
                "- 集成测试放在tests/test_integration.py\n"
                "- 不要mock内部模块、数据库层、路由或模型\n"
                "- 使用真实数据库（临时文件）、真实app实例、真实HTTP客户端\n"
                "- 不要mock任何东西，除非它确实不存在或需要人工交互。"
                "使用真实数据库、真实HTTP客户端、真实文件I/O、真实子进程调用。"
                "唯一应该mock的是：用户输入（键盘/鼠标）。如果系统上安装了库或服务，真实使用它。\n"
                "- 绝不mock .marvin/upstream/中的任何东西。如果upstream CLI可用，用真实子进程测试。\n"
                "- 测试通过实际FastAPI应用的完整请求→响应流程：\n"
                "  • 通过API创建会话，发送消息，验证SSE流\n"
                "  • 创建、列表、获取、删除会话——验证DB状态匹配\n"
                "  • 验证设置更新持久化并影响后续请求\n"
                "  • 验证错误响应有正确的状态码和格式\n"
                "  • 验证操作后数据库确实包含预期的行\n"
                "  • 验证静态文件服务正常（index.html、CSS、JS）\n"
                "- 使用httpx.AsyncClient配合ASGITransport进行真实HTTP测试\n"
                "- 测试必须失败（实现还不存在）\n"
                "- 不要编写任何实现代码——只写测试\n"
                "- 不要尝试运行测试——它们预期会失败\n\n"
                "用消息'Add failing integration tests (TDD red phase)'提交测试文件。"
            )
    ...
    if _phase_done("3"):
        ...
        if os.path.isfile(design_path):
            ...
            ux_path = os.path.join(wd, ".marvin", "ux.md")
            if os.path.isfile(ux_path):
                impl_prompt += "  - .marvin/ux.md（UX设计、组件、样式指南）\n"
            impl_prompt += (
                "  - .marvin/design.md（架构和实现计划）\n\n"
                "严格遵循这些文档中指定的架构、文件结构和UX设计。"
                "除非遇到技术上的不可能，不要偏离。\n\n"
                "字符串文字中的内联代码——转义规则：\n"
                "如果你在TypeScript/Python字符串文字中嵌入JavaScript/CSS"
                "（例如从服务器提供内联JS、带<script>块的HTML模板），"
                "所有反斜杠序列必须双重转义。例如：\n"
                "  - 匹配换行的正则必须是\\\\n而不是\\n（\\n会变成真正的换行）\n"
                "  - 匹配tab的正则必须是\\\\t而不是\\t\n"
                "  - 匹配词边界的正则必须是\\\\b而不是\\b\n"
                "  - 正则中的反斜杠必须是\\\\\\\\而不是\\\\\n"
                "这适用于所有作为字符串提供的代码：内联<script>、"
                "通过res.send()发送的模板文字、包含JS/CSS/HTML的字符串常量。"
                "如果宿主语言解释了转义，提供的代码将会损坏。\n\n"
                "视觉打磨至关重要：CSS必须看起来专业和现代——不是带颜色的无样式HTML。"
                "严格遵循.marvin/ux.md中的UX设计：使用指定的box-shadow、border-radius、"
                "transition、hover状态和间距比例。每个交互元素需要可见的hover/focus过渡。"
                "排版必须有清晰的层次。留白应该感觉有意且宽敞。"
                "如果规格定义了渐变、backdrop-filter或动画，实现它们。"
                "标准是Linear/Vercel/Raycast级别的精工细作。"
            )
        if params.tdd:
            impl_prompt += (
                "\n\nTDD模式：失败的测试已经编写好了。"
                "读取测试文件了解预期行为，然后实现代码使其通过。"
                "实现后用run_command运行测试验证通过。"
            )
        ...
        impl_prompt += (
            "\n\n提问协议：如果你需要关于项目架构、Marvin CLI接口或组件集成方式的澄清，"
            "可以只回复'QUESTIONS_FOR_PARENT:'后跟编号问题。"
            "父编排器会回答并带着答案重新运行你。"
            "谨慎使用——只用于真正的阻塞问题，而非通过阅读spec/design文档能解决的事。"
        )
...
async def review_codebase(params: ReviewCodebaseParams) -> str:
    ...
    review_prompt_r1 = (
        ...
        "你的工作：找出代码偏离文档意图、有缺陷、安全问题、"
        "缺少错误处理或规格违反的地方。\n\n"
        "方法：\n"
        "1) 先读参考文档（分块——使用start_line/end_line）。用write_note保存关键发现。\n"
        "2) 读取每个代码文件。与参考文档预期对比。\n"
        "3) 报告发现的问题。\n\n"
        "审查标准——只标记真正的问题：\n"
        "1. **规格违反** — 代码不匹配文档描述的行为\n"
        "2. **安全** — XSS、注入、不安全操作、缺少转义\n"
        "3. **逻辑缺陷** — 条件错误、边界错误、竞态条件、数据丢失\n"
        "4. **缺少错误处理** — 未处理的异常、缺少null检查\n"
        "5. **集成缺陷** — 错误的API格式、协议不匹配\n"
        "6. **测试缺口** — 未测试的关键路径、不应mock的东西被mock了\n\n"
        "不要标记：代码风格、命名、小重构、缺少注释。\n\n"
        "每个问题格式：FILE:LINE — SEVERITY (critical/major/minor) — 描述\n\n"
        "首轮审查——代码永远不完美。找出每个问题。"
        "以REVIEW_FAILED + 问题数量结尾。\n"
        "只读——不编辑、不执行命令。\n"
    )
    ...
    review_prompt_quality = (
        ...
        "2) 健壮性——错误处理、边界情况？"
        "3) 一致性——命名、模式、约定统一？"
        "4) 性能——明显的N+1查询、阻塞I/O、内存泄漏？"
        "5) 可测试性——代码能否在不过度mock的情况下测试？\n\n"
        "每个问题格式：FILE:LINE — SEVERITY (critical/major/minor) — 描述\n\n"
        "找出每个问题。以REVIEW_FAILED + 数量结尾。\n"
        "只读——不编辑、不执行命令。\n"
    )
    ...
    review_prompt_r2 = (
        ...
        "仍然存在于代码中，即为失败。不得放过。\n\n"
        "修复代理可能声称已修复——忽略其声明。"
        "只有实际代码才重要。通过阅读相关行来验证每个修复。\n\n"
        "如果所有critical+major问题已修复→回复：REVIEW_CLEAN\n"
        "如果任何问题仍存在→列出问题及SEVERITY + REVIEW_FAILED。严格审查。\n"
    )
    ...
    for review_round in range(1, params.max_rounds + 1):
        ...
        fix_prompt = (
            ...
            "4) 对每个问题执行apply_patch——每个问题一个补丁\n"
            "5) 停止。不要总结。不要自我审查。不要评分。\n\n"
            "规则：\n"
            "- 修复每一个critical和major问题。不得跳过。\n"
            "- 参考文档解释意图——使代码匹配文档描述的行为。\n"
            "- 不要分析、总结、评分或审查自己的工作。\n"
            "- 只使用：read_file、apply_patch。不用其他工具。\n"
            "- 另有独立审查者检查你的工作。\n"
        )
