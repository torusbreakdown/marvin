...
def _resolve_coding_path(rel_path: str) -> str:
    ...
    if rel_resolved == ".tickets" or rel_resolved.startswith(".tickets" + os.sep):
        raise ValueError(
            f"Cannot edit .tickets/ files directly. Use the tk tool instead:\n"
            f"  tk(args='create \"Title\" -t task --parent PARENT_ID')  # create ticket\n"
            f"  tk(args='start ID')       # mark in-progress\n"
            f"  tk(args='close ID')       # mark done\n"
            f"  tk(args='add-note ID \"text\"')  # add a note\n"
            f"  tk(args='show ID')        # view ticket\n"
            f"  tk(args='ls')             # list tickets"
        )
    return resolved
...
def _acquire_lock(directory: str, tool_name: str) -> str | None:
    """No-op — locking disabled. Agents run serially for write tasks."""
    return None


def _release_lock(directory: str):
    """No-op — locking disabled."""
    pass
...
class CreateFileParams(BaseModel):
    ...
    content: str = Field(default="", description="File content to write. REQUIRED — include the full file content.")


class AppendFileParams(BaseModel):
    path: str = Field(description="File path relative to working directory (no absolute paths)")
    content: str = Field(default="", description="Content to append to the file. REQUIRED.")


class ApplyPatchParams(BaseModel):
    path: str = Field(description="File path relative to working directory (no absolute paths)")
    old_str: str = Field(default="", description="Exact string to find in the file (must match exactly). REQUIRED.")
    new_str: str = Field(default="", description="Replacement string. REQUIRED (use empty string to delete).")
...
def _ticket_gate_check() -> str | None:
    ...
    return (
        "⚠️ BLOCKED: You must create a ticket before doing any work.\n"
        f"Use tk(args='create \"Your task title\" -t task --parent {parent} "
        f"-d \"Detailed description with acceptance criteria\"') first.\n"
        "Your first tk create will be rejected once for review — this is expected. "
        "Retry with a thorough description including acceptance criteria."
    )
...
async def create_file(params: CreateFileParams) -> str:
    if not params.content:
        return (
            "❌ ERROR: 'content' parameter is missing or empty. "
            "create_file requires BOTH 'path' and 'content' parameters.\n"
...
@define_tool(
    description=(
        "Append content to an existing file (file must already exist — use create_file first). "
        "Parameters: path (relative to working dir), content (text to append — REQUIRED, must not be empty). "
        "Use this after create_file to write large documents in sections, keeping each call under ~3000 words."
    )
)
async def append_file(params: AppendFileParams) -> str:
    if not params.content:
        return (
            "❌ ERROR: 'content' parameter is missing or empty. "
    ...
    if not os.path.isfile(full):
        return f"File not found: {full}. Use create_file first."
    ...
    total_lines = sum(1 for _ in open(full))
    return f"✅ Appended to {full} (now {total_lines} lines total)"
...
async def apply_patch(params: ApplyPatchParams) -> str:
    if not params.old_str:
        return (
            "❌ ERROR: 'old_str' parameter is missing or empty.\n"
            "apply_patch requires: path, old_str, new_str.\n"
            "old_str must be the EXACT text from the file you want to replace.\n"
...
async def read_file(params: ReadFileParams) -> str:
    ...
    selected_size = sum(len(l) for l in lines[start:end])
    if selected_size > _MAX_READ_BYTES and not (params.start_line or params.end_line):
        return (
            f"⚠️ File is too large to read in full ({total} lines, ~{os.path.getsize(full)//1024}KB). "
            f"Use start_line and end_line to read a section at a time.\n"
            f"Example: read_file(path='{params.path}', start_line=1, end_line=200)\n"
            f"Then: read_file(path='{params.path}', start_line=201, end_line=400)\n"
            f"Total lines: {total}"
        )
...
async def run_command(params: RunCommandParams) -> str:
    ...
    if cmd_stripped.startswith("tk ") or cmd_stripped == "tk":
        return (
            ...
            "The tk tool provides structured ticket management. Example:\n"
            "  tk(args='create \"Title\" -t task -d \"Description with acceptance criteria\" --parent PARENT_ID')\n"
            "  tk(args='start ID')\n"
            "  tk(args='close ID')\n"
            "  tk(args='add-note ID \"note\"')\n"
            "  tk(args='ls')"
        )
...
async def tk(params: TkParams) -> str:
    ...
    # For 'create' commands, unconditionally reject the first call so the agent
    # pauses to reconsider. Accept unconditionally on the second call.
    if args and args[0] == "create":
        if not hasattr(tk, "_create_rejected"):
            tk._create_rejected = False
        if not tk._create_rejected:
            tk._create_rejected = True
            parent_ticket = os.environ.get("MARVIN_TICKET", "")
            return (
                "⚠️ REJECTED — review your ticket before creating it.\n"
                "Before retrying, verify:\n"
                "1. Does the title clearly describe the deliverable?\n"
                "2. Did you include -d with a DETAILED description that includes:\n"
                "   - What exactly will be done\n"
                "   - Acceptance criteria (bullet list of testable outcomes)\n"
                "   - Key files to create or modify\n"
                "   - Dependencies on other work\n"
                f"3. Did you set --parent {parent_ticket} (your parent epic)?\n"
                "4. Did you set -t to the right type (epic for phases, task for work items)?\n"
                "5. Did you add --tags for categorization?\n\n"
                "If you're confident this is correct, call tk again with the same or improved args."
            )
        # On successful create, mark that this agent has created a ticket
        ok, output = _run_cmd(["tk"] + args, timeout=10, cwd=wd)
        if ok:
            tk._agent_has_ticket = True
...
async def launch_agent(params: LaunchAgentParams) -> str:
    ...
    def _project_context() -> str:
        ...
        parts.append(
            "TICKET TRACKING — use the tk tool (DO NOT edit .tickets/ files directly):\n"
            ...
            "  link <id> <id>           Link tickets together\n"
            "  ls [--status=X]          List tickets\n"
            "  show <id>                Display ticket\n"
            "  add-note <id> [text]     Append timestamped note\n\n"
            "How to use tickets in this pipeline:\n"
            f"1. Create an EPIC for your phase: tk(args='create \"Phase: X\" -t epic --parent {ticket_id}')\n"
            "2. Create TASKs under the epic for each unit of work: tk(args='create \"Write tests for API\" -t task --parent EPIC_ID')\n"
            "3. Start tasks when you begin: tk(args='start TASK_ID')\n"
            "4. Add notes for findings/decisions: tk(args='add-note TASK_ID \"Found 3 issues\"')\n"
            "5. Close tasks when done: tk(args='close TASK_ID')\n"
            "6. Close the epic when all tasks are done: tk(args='close EPIC_ID')\n"
        )
    ...
    async def _code_review(phase_label: str, focus: str = "all") -> None:
        ...
        def _build_review_prompt(area: str, files: list[str]) -> str:
            ...
            ctx += (
                f"You are a strict CODE REVIEWER auditing the '{phase_label}' phase — "
                f"review ONLY **{area}** files.\n\n"
                "Read each file below with read_file. Also read .marvin/spec.md, "
                ".marvin/ux.md, and .marvin/design.md to understand expected behavior.\n\n"
                "IMPORTANT: Do NOT review files in .marvin/upstream/ — those are upstream references. "
                "Do NOT review files outside the list below.\n\n"
                f"Files to review:\n{chr(10).join('  - ' + f for f in sorted(files))}\n"
            )
            if area == "Backend":
                ctx += (
                    f"{app_interface_hint}\n"
                    "Focus: backend integration, database correctness, API contract matching spec, "
                    "error handling, SQL injection, subprocess safety.\n\n"
                )
            elif area == "Frontend":
                ctx += (
                    "\nFocus: XSS in innerHTML/renderMarkdown, SSE parsing correctness, "
                    "fetch error handling, accessibility, keyboard navigation, theme switching, "
                    "responsive breakpoints matching spec.\n\n"
                )
            else:
                ctx += (
                    "\nFocus: test coverage gaps, unnecessary mocks (tests should use real implementations — "
                    "flag any mocking of modules/services that exist in the system), "
                    "fixture setup, assertion precision, tests asserting wrong behavior.\n\n"
                )
            ctx += (
                "Review criteria — only flag real issues:\n"
                "1. **Security** — XSS, injection, unsafe HTML, missing escaping\n"
                "2. **Logic bugs** — wrong conditions, off-by-one, race conditions\n"
                "3. **Integration bugs** — wrong endpoints, wrong data structures, SSE issues\n"
                ...
                "Do NOT flag: code style, naming, missing comments, minor refactors.\n\n"
                "Format for each issue: FILE:LINE — SEVERITY (critical/major/minor) — description\n\n"
                "Output findings as a report. Do NOT edit any files. Do NOT run commands.\n"
                "You are a read-only reviewer — report issues, the parent orchestrator will "
                "dispatch a separate agent to fix them.\n\n"
                "If there are no critical/major issues, respond 'REVIEW_CLEAN'."
            )
            return ctx
        ...
        if all_project_files:
            ...
            if is_test_only_phase:
                spec_review_ctx += (
                    "IMPORTANT: This is the TEST WRITING phase (TDD). Implementation code does not "
                    "exist yet — this is expected and correct. Do NOT report missing implementation "
                    "as an issue. Your job is ONLY to verify tests correctly cover spec requirements. "
                    "Tests SHOULD fail right now (no implementation) — that's the point of TDD.\n\n"
                    "Steps:\n"
                    "1. Read .marvin/spec.md — note every user story, acceptance criterion, endpoint, behavior.\n"
                    "2. Read .marvin/design.md — note the test plan section.\n"
                    "3. Read all test files and verify:\n"
                    "   - Tests cover each spec requirement and acceptance criterion\n"
                    "   - Tests verify observable behavior matching the spec\n"
                    "   - Tests don't mock things that should be tested for real\n"
                    "   - Test assertions match spec-defined values (status codes, field names, etc.)\n"
                    "   - Tests don't assert placeholder/dummy behavior\n\n"
                )
            else:
                spec_review_ctx += (
                    "Your ONLY job is to verify the implementation exactly matches the spec and "
                    "architecture documents. You do NOT review code quality — only check that every "
                    "requirement is correctly implemented.\n\n"
                    "Steps:\n"
                    "1. Read .marvin/spec.md in chunks (200 lines at a time) — note every user story, "
                    "acceptance criterion, endpoint, behavior, constraint, and functional requirement. "
                    "Use write_note to save findings.\n"
                    "2. Read .marvin/ux.md in chunks (200 lines at a time) — note every screen, component, "
                    "interaction, state, style rule, and accessibility requirement. Use write_note.\n"
                    "3. Read .marvin/design.md in chunks (200 lines at a time) — note every architecture "
                    "decision, file structure, API shape, database schema, and module responsibility. "
                    "Use write_note.\n"
                    "4. Read all implementation files and verify:\n"
                    "   - Each user story's acceptance criteria are implemented\n"
                    "   - Each API endpoint matches the specified request/response format\n"
                    "   - Each database table/column matches the schema\n"
                    "   - Each keyboard shortcut, theme rule, and UX behavior exists\n"
                    "   - Error handling matches spec requirements\n"
                    "   - File structure matches architecture\n"
                    "5. Read all test files and verify:\n"
                    "   - Tests cover each spec requirement (not just implementation details)\n"
                    "   - Tests don't mock things that should be tested for real\n"
                    "   - Tests verify observable behavior matching the spec\n"
                    "   - Tests don't assert wrong behavior that contradicts the spec\n\n"
                )
            spec_review_ctx += (
                f"Project files:\n{chr(10).join('  - ' + f for f in sorted(all_project_files))}\n\n"
                "Output format — for each finding:\n"
                "SPEC_GAP: <spec section> — <what is missing or wrong> — SEVERITY: critical/major/minor\n\n"
                "Output findings as a report. Do NOT edit any files. Do NOT run commands.\n"
                "You are a read-only reviewer — report issues, the parent orchestrator will "
                "dispatch a separate agent to fix them.\n\n"
                "If everything matches the spec, respond 'SPEC_CONFORMANT'."
            )
        ...
        for review_round in range(1, _MAX_REVIEW_ROUNDS + 1):
            ...
            if is_test_phase:
                fix_instructions = (
                    "INSTRUCTIONS (TDD red phase — tests SHOULD fail):\n"
                    "1. Read each file mentioned in the findings\n"
                    "2. Only fix test code quality issues: wrong assertions, missing edge cases, "
                    "bad test structure, missing fixtures, wrong imports\n"
                    "3. Do NOT write any implementation code — do not create server.py, app.ts, or source modules\n"
                    "4. Do NOT try to make tests pass — tests MUST fail because implementation doesn't exist. "
                    "This is TDD: tests define the interface before code is written.\n"
                    "5. Do NOT run pytest — tests will fail, that is correct.\n"
                    "6. If a reviewer says 'module X not found' or 'import error', this is EXPECTED in TDD. "
                    "Do not create modules. Do not fix imports. Leave them as-is.\n"
                    f"7. Commit fixes with message 'Test review fix ({phase_label.lower()}, round {review_round}): <summary>'\n\n"
                    "Read .marvin/spec.md, .marvin/ux.md, and .marvin/design.md to understand expected behavior."
                )
            else:
                fix_instructions = (
                    "INSTRUCTIONS:\n"
                    "1. Read each file mentioned in the findings\n"
                    "2. Fix each critical/major issue with minimal changes\n"
                    ...
                    "5. After committing, STOP. Do not self-review. Do not grade. Do not write summary reports.\n\n"
                    "Read .marvin/spec.md, .marvin/ux.md, and .marvin/design.md to understand expected behavior."
                )
    ...
    def _save_state(phase: str):
        """Save pipeline state, but never downgrade to an earlier phase."""
        current = _load_state()
        if current and phase in _phase_order and current in _phase_order:
            if _phase_order.index(current) >= _phase_order.index(phase):
    ...
    def _phase_done(phase: str) -> bool:
        ...
        if not current:
            return False
        try:
            return _phase_order.index(current) >= _phase_order.index(phase)
        except ValueError:
            return False
    ...
    _resumed_from = _load_state()
    if _resumed_from:
        await _notify_pipeline(f"🔄 Resuming pipeline from after phase {_resumed_from}")
    ...
    if params.design_first:
        ...
        if _phase_done("1a") or (os.path.isfile(spec_path) and os.path.getsize(spec_path) > 1000
                                and os.path.isfile(ux_path) and os.path.getsize(ux_path) > 1000):
            ...
            await _notify_pipeline("🎨 Phase 1a: Spec + UX design")
            ...
            # Spec agent — skip if spec.md already exists
            if os.path.isfile(spec_path) and os.path.getsize(spec_path) > 1000:
                await _notify_pipeline(f"⏭️ spec.md exists ({os.path.getsize(spec_path)}B), skipping spec agent")
            else:
                ...
                for _spec_attempt in range(1, _MAX_RETRIES + 1):
                    ...
                    if os.path.isfile(spec_path) and os.path.getsize(spec_path) > 1000:
                        ...
                        break
                    await _notify_pipeline(f"⚠️ spec.md not written (attempt {_spec_attempt}/{_MAX_RETRIES})")
                else:
                    await _notify_pipeline(f"🚫 Pipeline ABORTED: product spec failed after {_MAX_RETRIES} attempts")
            ...
            for _ux_attempt in range(1, _MAX_RETRIES + 1):
                ...
                if os.path.isfile(ux_path) and os.path.getsize(ux_path) > 1000:
                    ...
                    break
                await _notify_pipeline(f"⚠️ ux.md not written (attempt {_ux_attempt}/{_MAX_RETRIES})")
            else:
                await _notify_pipeline(f"🚫 Pipeline ABORTED: UX design failed after {_MAX_RETRIES} attempts")
        ...
        if _phase_done("1b") or (os.path.isfile(design_path) and os.path.getsize(design_path) > 1000):
            ...
            arch_prompt = (
                "You are a SENIOR ARCHITECT. Read .marvin/spec.md and .marvin/ux.md in chunks "
                "with read_file (200 lines at a time to manage context budget), then write the "
                "architecture and test plan. Do NOT write code.\n\n"
                "Write the entire document in English.\n\n"
                "⚠️ CONTEXT BUDGET: These files are large (~200KB total). "
                "Read strategically — scan structure first (lines 1-200), then read key sections. "
                "Use write_note between reads to save findings. Do not try to read both files in full.\n\n"
                "Include:\n"
                "1) Architecture — file structure, modules, data flow, API endpoints "
                "(request/response shapes + status codes), database schema, error handling, "
                "tech choices with rationale\n"
                "2) Implementation plan — files listed in order with responsibilities + spec requirement tracing\n"
                "3) Test plan (MOST IMPORTANT) — for every module/endpoint/component/flow:\n"
                "   - Test file path + each test function name + what it verifies\n"
                "   - Coverage: happy path, error, boundary, edge, invalid, null, concurrent\n"
                "   - Every public function, API route, and error handler must have tests\n"
                "   - Target 100%% functional coverage\n\n"
                "NO MOCKING: Use real database (SQLite temp), real HTTP clients, real file I/O, "
                "real subprocesses. Only mock user keyboard input. No fake fixtures, no 'mock' in "
                "test names, no indirect command-argument calls. Test observable behavior "
                "(HTTP responses, database state, rendered output) not internal implementation.\n\n"
            )
            upstream_summary = _upstream_summary()
            if upstream_summary:
                arch_prompt += (
                    "UPSTREAM (.marvin/upstream/) = authoritative docs of existing system. "
                    "Hard requirements, not suggestions. Must match exactly. Read all with read_file before writing.\n\n"
                )
            if instructions_ctx:
                arch_prompt += f"PROJECT INSTRUCTIONS:\n{instructions_ctx}\n\n"
            arch_prompt += (
                "Save as .marvin/design.md. Write in segments: first with create_file, then "
                "append_file (2000-4000 words each). This document + spec + UX = the ONLY reference "
                "for all implementation agents. The test plan must be exhaustive enough that an agent "
                "can write the complete test suite from it alone."
            )
            ...
            for _arch_attempt in range(1, _MAX_RETRIES + 1):
                ...
                if os.path.isfile(design_path) and os.path.getsize(design_path) > 1000:
                    ...
                    break
                # Delete garbage if agent wrote CoT instead of real content
                if os.path.isfile(design_path):
                    os.remove(design_path)
                await _notify_pipeline(f"⚠️ design.md not written (attempt {_arch_attempt}/{_MAX_RETRIES})")
            else:
                await _notify_pipeline(f"🚫 Pipeline ABORTED: architecture pass failed after {_MAX_RETRIES} attempts")
                return f"🚫 Architecture pass failed (exit {rc}, ticket {params.ticket_id}):\n{derr or dout}"
        _save_state("1b")

        # Phase 1b-review: verify design against upstream references AND spec
        if _phase_done("1b_review"):
        ...
        if _phase_done("2a"):
            ...
            for i, batch in enumerate(batches):
                ...
                test_prompt = (
                    ...
                    "- DO NOT MOCK ANYTHING unless it literally does not exist or requires "
                    "human interaction. Use real databases, real HTTP clients, real file I/O, "
                    "real subprocess calls. The only things you should mock are: user input "
                    "(keyboard/mouse) and services that "
                    "aren't installed on this system. If a library is installed, USE IT FOR REAL.\n"
                    "- NEVER mock internal modules, database calls, the filesystem, or anything "
                    "in .marvin/upstream/. If an upstream CLI is available locally, test "
                    "against it with a real subprocess.\n"
                    "- Tests must verify OBSERVABLE BEHAVIOR (HTTP responses, DB state, rendered "
                    "output), not internal implementation details. If a test passes when the "
                    "behavior is wrong, the test is broken.\n"
            ...
            if failures:
                await _notify_pipeline(f"⚠️ {len(failures)}/{len(test_results)} test agents failed, continuing with partial results")
    ...
    if params.tdd:
        if _phase_done("2b"):
            ...
            integ_prompt = (
                "You are writing integration tests for a TDD workflow. Read the spec .marvin/spec.md, "
                "UX design .marvin/ux.md, and architecture doc .marvin/design.md.\n\n"
                "You are writing tests that verify the real system works end-to-end. "
                "These tests complement the existing (or upcoming) functional tests.\n\n"
                ...
                "- Use real database (temp file), real app instance, real HTTP client\n"
                "- Do NOT mock anything unless it truly doesn't exist or requires human interaction. "
                "Use real database, real HTTP client, real file I/O, real subprocess calls. "
                "The ONLY thing you should mock is: user input (keyboard/mouse). "
                "If a library or service is installed on the system, use it for real.\n"
                "- NEVER mock anything in .marvin/upstream/. If the upstream CLI is available, "
                "test it with real subprocesses.\n"
                "- Test the full request→response flow through the actual app:\n"
                "  • Create sessions via API, send messages, verify SSE streams\n"
                "  • Create, list, get, delete sessions — verify DB state matches\n"
                ...
                "  • Verify the database actually contains expected rows after operations\n"
                ...
                "- Do NOT write any implementation code — only tests\n"
                "- Do NOT try to run tests — they are expected to fail\n\n"
                "Commit test files with message 'Add failing integration tests (TDD red phase)'."
            )
    ...
    if _phase_done("3"):
        ...
        if os.path.isfile(design_path):
            ...
            if os.path.isfile(spec_path):
                impl_prompt += "  - .marvin/spec.md (product spec and functional requirements)\n"
            ux_path = os.path.join(wd, ".marvin", "ux.md")
            if os.path.isfile(ux_path):
                impl_prompt += "  - .marvin/ux.md (UX design, components, style guide)\n"
            impl_prompt += (
                "  - .marvin/design.md (architecture and implementation plan)\n\n"
                "Follow the architecture, file structure, and UX design specified in these "
                "documents STRICTLY. Do not deviate unless technically impossible.\n\n"
                "INLINE CODE IN STRING LITERALS — escaping rules:\n"
                "If you embed JavaScript/CSS in TypeScript/Python string literals "
                "(e.g. serving inline JS from server, HTML templates with <script> blocks), "
                "all backslash sequences must be double-escaped. For example:\n"
                "  - Regex matching newline must be \\\\n not \\n (\\n becomes a real newline)\n"
                "  - Regex matching tab must be \\\\t not \\t\n"
                "  - Regex matching word boundary must be \\\\b not \\b\n"
                "  - Backslash in regex must be \\\\\\\\ not \\\\\n"
                "This applies to ALL code served as strings: inline <script>, "
                "template literals sent via res.send(), string constants containing JS/CSS/HTML. "
                "If the host language interprets the escapes, the served code will be corrupted.\n\n"
                "VISUAL POLISH IS CRITICAL: CSS must look professional and modern — not unstyled "
                "HTML with colors. Follow the UX design in .marvin/ux.md strictly: use the specified "
                "box-shadow, border-radius, transition, hover states, and spacing ratios. Every "
                "interactive element needs visible hover/focus transitions. Typography must have "
                "clear hierarchy. Whitespace should feel intentional and spacious. "
                "If the spec defines gradients, backdrop-filter, or animations, implement them. "
                "The standard is Linear/Vercel/Raycast level craftsmanship."
            )
        if params.tdd:
            impl_prompt += (
                "\n\nTDD MODE: Failing tests have already been written. "
                "Read the test files to understand expected behavior, then implement code "
                "to make them pass. After implementing, run tests with run_command to verify."
            )
        ...
        impl_prompt += (
            ...
            "Marvin CLI interface, or how components integrate, you can respond with just "
            "'QUESTIONS_FOR_PARENT:' followed by numbered questions. "
            "The parent orchestrator will answer and re-run you with the answers. "
            "Use sparingly — only for truly blocking questions, not things that can be "
            "resolved by reading the spec/design documents."
        )
        ...
        if _phase_done("4a"):
            ...
            for debug_round in range(1, max_debug_rounds + 1):
                ...
                debug_prompt = (
                    ...
                    "INLINE CODE ESCAPING: If fixing JS/CSS embedded in string literals "
                    "(inline <script>, template strings served as HTML), remember that backslash "
                    "sequences like \\n, \\t, \\b are interpreted by the HOST language. Use "
                    "double-escaping (\\\\n, \\\\t) so the served code contains the literal backslash."
                ...
                if "ALL_TESTS_PASS" in (dbg_out or ""):
                    ...
                    break
                if rc != 0:
                    await _notify_pipeline(f"⚠️ Debug round {debug_round} failed (exit {rc})")
            else:
        ...
        if _phase_done("4b"):
            ...
            for e2e_round in range(1, max_e2e_rounds + 1):
                e2e_prompt = (
                    ...
                    "(the upstream system may fail but the endpoint should respond, not 500)\n"
                ...
                # Add upstream interface context so E2E agent understands integration
                ...
                if "E2E_SMOKE_PASS" in (e2e_out or ""):
                    ...
                    break
                if rc != 0:
                    await _notify_pipeline(f"⚠️ E2E round {e2e_round} failed (exit {rc})")
            else:
            ...
            qa_areas = [
                ...
                },
                {
                    ...
                    "focus": (
                        ...
                        "- Check that backend subprocesses are killed on client disconnect\n"
...
async def write_note(params: WriteNoteParams) -> str:
    ...
    # In coding mode, redirect notes to .marvin/notes/ inside the project
    if _coding_working_dir:
        notes_dir = os.path.join(_coding_working_dir, ".marvin", "notes")
    else:
        notes_dir = _NOTES_DIR
    full = os.path.join(notes_dir, rel)
...
async def create_ticket(params: CreateTicketParams) -> str:
    ...
    ok2, details = _run_cmd(["tk", "show", ticket_id], timeout=5, cwd=_coding_working_dir)
    if ok2:
...
async def ticket_add_dep(params: TicketDepParams) -> str:
    ok, out = _run_cmd(["tk", "dep", params.ticket_id, params.depends_on], timeout=5, cwd=_coding_working_dir)
...
async def ticket_start(params: TicketStatusParams) -> str:
    ok, out = _run_cmd(["tk", "start", params.ticket_id], timeout=5, cwd=_coding_working_dir)
    return out.strip() if ok else f"Failed: {out}"
...
async def ticket_close(params: TicketStatusParams) -> str:
    ok, out = _run_cmd(["tk", "close", params.ticket_id], timeout=5, cwd=_coding_working_dir)
    return out.strip() if ok else f"Failed: {out}"
...
async def ticket_show(params: TicketStatusParams) -> str:
    ok, out = _run_cmd(["tk", "show", params.ticket_id], timeout=10, cwd=_coding_working_dir)
...
async def ticket_list(params: TicketListParams) -> str:
    ...
    if params.tags:
        cmd += ["-T", params.tags]
    ok, out = _run_cmd(cmd, timeout=10, cwd=_coding_working_dir)
    return out if ok else f"Failed: {out}"
...
async def ticket_dep_tree(params: TicketTreeParams) -> str:
    ok, out = _run_cmd(["tk", "dep", "tree", params.ticket_id], timeout=10, cwd=_coding_working_dir)
    return out if ok else f"Failed: {out}"
...
async def _run_non_interactive():
    ...
    if design_first:
        ok, out = _run_cmd(["tk", "create", prompt_text, "-t", "epic", "-p", "0"], timeout=5, cwd=_coding_working_dir)
